# nuke-k3s-prod-master-0 (.37) — new prod cluster master (k3s server, SQLite, tainted).
...
# live on the node object (from install); a k3s restart preserves them. Cluster is
# ISOLATED (docker.prod.nuke → .31), so a k3s restart on apply has no trading impact.
